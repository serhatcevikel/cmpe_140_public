...
# and put spaces after commas separating the arguments,
# and spaces before and after equal signs (=) that introduce default values for arguments
# use curly braces to delimit blocks for functions, loops and condititonals
# it is best to put opening and closing curly braces in separate lines just below the line initiating the block
# and leave them alone so that you can track the blocks that you open and close easier
# in a well written code, the opening and closing curly braces for a block are on the same indent level

# the curly brace that introduces the outermost block for the function should be at the beginning of the line,
# with no leading indents
{
...
# a level of indent should be introduced by a tab, not single spaces
    ...
    # and make sure that in each "for" or "while" loop,
    # you introduce a variable name that was not previously used inside the function
    # and make sure not to refer to global variable: variables outside the scope of your function
    # (e.g. variables that were introduced before you started your function)
    # if you have to use those global variables, pass them using arguments of your function
  
    for (factorx in 2:sqrt(number)) # for1: inside the comments you can enumerate nested "for" or "while" loops as such:
    # for1, for2, etc, to better track them
    {
